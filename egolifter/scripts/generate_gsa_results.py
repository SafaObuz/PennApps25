...
class ColmapDataset(Dataset):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # PATCH 
        self.rgb_paths = natsorted(
            glob.glob(str(args.input_folder / "images" / "*.JPG")) +
            glob.glob(str(args.input_folder / "images" / "*.jpg")) +
            glob.glob(str(args.input_folder / "images" / "*.png"))
        )
...
def main(args: argparse.Namespace):
    ...
    clip_model, _, clip_preprocess = open_clip.create_model_and_transforms(
        "ViT-H-14",
        pretrained="/home/safaobuz/.cache/huggingface/hub/models--laion--CLIP-ViT-H-14-laion2B-s32B-b79K/blobs/open_clip_pytorch_model.bin"
    )

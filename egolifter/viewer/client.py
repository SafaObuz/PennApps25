...
def _safe_atomic(client):
    """Safe atomic context that handles None client"""
            ...
            with torch.no_grad():
                try:
                    ...
                    if torch.isnan(image).any() or torch.isinf(image).any():
                        print("[ClientThread] Warning: Invalid values in rendered image")
                    ...
                    # Ensure canvas is valid before sending
                    if torch.isnan(canvas).any() or torch.isinf(canvas).any():
                        print("[ClientThread] Warning: Invalid values in canvas")
                    ...
                    if "CUDA" in str(e):
                        print(f"[ClientThread] CUDA error during rendering: {e}")

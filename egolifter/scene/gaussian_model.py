...
class GaussianModel(nn.Module):
    ...
    def load_ply(self, path, og_number_points=-1):
        ...
        self._features_rest = nn.Parameter(torch.tensor(features_rest, dtype=torch.float, device=get_device()).transpose(1, 2).contiguous().requires_grad_(True))
        self._features_extra = nn.Parameter(torch.tensor(features_extra, dtype=torch.float, device=get_device()).contiguous().requires_grad_(True))
    ...
    def delete_gaussians(self, mask):
        ...
        Args:
            mask: Boolean tensor of shape (N,) indicating which Gaussians to delete
        """
        if not mask.any():
            return
        ...
        try:
            ...
            if torch.isnan(mask.float()).any():
                print("[GaussianModel] Warning: Invalid values in deletion mask")
                return
            
            # Keep only the Gaussians that are NOT marked for deletion
            keep_mask = ~mask
            
            # Update all Gaussian parameters
            self._xyz.data = self._xyz[keep_mask]
            self._features_dc.data = self._features_dc[keep_mask]
            self._features_rest.data = self._features_rest[keep_mask]
            if self._features_extra is not None:
                self._features_extra.data = self._features_extra[keep_mask]
            self._scaling.data = self._scaling[keep_mask]
            self._rotation.data = self._rotation[keep_mask]
            self._opacity.data = self._opacity[keep_mask]
            ...
            # Clear CUDA cache after tensor operations
            torch.cuda.empty_cache()
            
            print(f"[GaussianModel] Deleted {mask.sum().item()} Gaussians, {keep_mask.sum().item()} remaining")
        except Exception as e:
            print(f"[GaussianModel] Error deleting Gaussians: {e}")
